...
export default new Vuex.Store({
  ...
  actions: {
    ...
    async removeQuiz({ dispatch },form){
      const { std, subject:newSubject, question, correct_answer, incorrect_answer} = form
      let newObject = {};
      ...
      await fb.quizCollection.doc(std).update(newObject)
      .then().catch((err) => {
    ...
    async addQuiz({ commit,dispatch },form){
      const { std, subject:newSubject, question, correct_answer, incorrect_answer} = form
      if(std !== '' && newSubject !== '' && question !== '' && correct_answer !== '' &&
       incorrect_answer.option1 !== '' && incorrect_answer.option2 !== '' && incorrect_answer.option3 !== ''){
      let newObject = {};
      newObject[newSubject] = fb.firestore.FieldValue.arrayUnion({
        correct_answer:correct_answer,
        question:question,
        incorrect_answer:[incorrect_answer.option1,incorrect_answer.option2,incorrect_answer.option3]
      });
      await fb.quizCollection.doc(std).update(newObject)
      .then((() =>{
        commit('setLoading',{
    ...
    async removeNotes({ dispatch },form){
      ...
      await fb.notesCollection.doc(std).update(newObject)
      .then().catch((err) => {
        console.log(err.message)
      })
      await storageRef.child('notes/' + std +'/' + newSubject +'/' + topicName + '.pdf').delete()
      .then(function(){
    ...
    async addNotes({ commit,dispatch },form){
      ...
      await storageRef.child('notes/' + std +'/' + newSubject +'/' + topicName + '.pdf').put(file)
      .then(function(){
        console.log("Success")
      })
    }
    ...
    async removeNews({ dispatch },form){
      ...
      await fb.newsCollection.doc('d9qYOow2oLfzN1M1En6c').update({
    ...
    async removeVideo({ dispatch },form){
      ...
      await fb.videoCollection.doc(std).update(newObject)
      .then().catch((err) => {
        console.log(err.message)
      })
    ...
    async addVideo({ commit,dispatch },form){
      const {std, subject: newSubject, topicName, videoLink } = form
      if(std !== '' && newSubject !== '' && topicName !== '' && videoLink !== ''){
      let newObject = {};
      newObject[newSubject] = fb.firestore.FieldValue.arrayUnion({
        topicName:topicName,
        videoLink:videoLink
    ...
    async fetchQuiz({ commit,dispatch },filter){
      ...
      await fb.quizCollection.doc(filter.class).get()
      .then((fetchQuiz) =>{
        var quizes = {}
    ...
    async uploadProfileImage({dispatch},form){
      const userId = fb.auth.currentUser.uid
      await storageRef.child('users/' + userId + '/profile.jpg').put(form.file)
      .then(function(){
    ...
    async downloadProfileImage({ commit }){
      const user = fb.auth.currentUser
      if(user !== null){
      await storageRef.child('users/' + user.uid + '/profile.jpg').getDownloadURL()
      .then(imgUrl => {
    ...
    async fetchUserProfile({ commit }, user) {
      ...
      if (router.currentRoute.path === '/' && userProfile.data().admin) {
       ...
       if(router.currentRoute.path === '/' && userProfile.data().admin === undefined){
    ...
    async fetchVideos({ commit }, filter){
      if(filter.class !== '' && filter.subject !== ''){
      await fb.videoCollection.doc(filter.class).get()
      .then((fetchVideos) =>{
        var videos = {}
    ...
    async fetchNotes({ commit }, filter){
      if(filter.class !== '' && filter.subject !== ''){
      await fb.notesCollection.doc(filter.class).get()
      .then((fetchNotes) =>{
        var notes = {}
    ...
    async fetchNotesLink({ commit }, form){
      await storageRef.child('notes/' + form.class +'/' + form.subject +'/' + form.topicName + '.pdf').getDownloadURL()
      .then(noteUrl => {       
